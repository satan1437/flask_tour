...
def departures(departure):
	...
	for i in tours_n:
		if tours_n[i]["departure"] == departure:
			count += 1
			if tours_n[i]["price"] > cost_max:
				cost_max = tours_n[i]["price"]
			if tours_n[i]["price"] < cost_min:
				cost_min = tours_n[i]["price"]
			if tours_n[i]["nights"] > n_max:
				n_max = tours_n[i]["nights"]
			if tours_n[i]["nights"] < n_min:
				n_min = tours_n[i]["nights"]
	temp = [count, cost_min, cost_max, n_min, n_max]
	return render_template('departure.html', data=tours_n, rgn=departure, temp=temp)
